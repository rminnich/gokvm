...
// code; see trace/kvm/ioctl_arm64.go), which independently confirms
// the same KVM_GET_REGS/KVM_SET_REGS ioctl numbers and ONE_REG
// register-ID encoding used here (see one_reg_arm64.go). voodoo's own
// Regs/GetRegs/SetRegs are shaped around its ptrace-register-set
// bridge rather than a direct mirror of struct kvm_regs (their own
// comments note the register-index-to-ID mapping is an approximation
// for that purpose), so this file defines the struct fresh, laid out
// to match the kernel header exactly, for use with the bulk
// KVM_GET_REGS/KVM_SET_REGS ioctls.
type Regs struct {
