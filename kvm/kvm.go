...
// GetAPIVersion gets the kvm API version.
...
// GetVCPUMMmapSize returns the size of the VCPU region.
...
func CreateDev(vmFd uintptr, dev *Device) error {
	_, err := Ioctl(vmFd, IIOWR(kvmCreateDev, unsafe.Sizeof(Device{})), uintptr(unsafe.Pointer(dev)))
...
func Translate(vcpuFd uintptr, t *Translation) error {
	_, err := Ioctl(vcpuFd, IIOWR(kvmTranslate, unsafe.Sizeof(Translation{})), uintptr(unsafe.Pointer(t)))
...
func GetMPState(vcpuFd uintptr, mps *MPState) error {
	_, err := Ioctl(vcpuFd, IIOR(kvmGetMPState, unsafe.Sizeof(MPState{})), uintptr(unsafe.Pointer(mps)))
...
func SetMPState(vcpuFd uintptr, mps *MPState) error {
	_, err := Ioctl(vcpuFd, IIOW(kvmSetMPState, unsafe.Sizeof(MPState{})), uintptr(unsafe.Pointer(mps)))
...
// GetVCPUEvents gets currently pending exceptions, interrupts, and NMIs.
func GetVCPUEvents(vcpuFd uintptr, event *VCPUEvents) error {
	_, err := Ioctl(vcpuFd, IIOR(kvmGetVCPUEvents, unsafe.Sizeof(VCPUEvents{})), uintptr(unsafe.Pointer(event)))

	return err
}

// SetVCPUEvents sets pending exceptions, interrupts, and NMIs.
func SetVCPUEvents(vcpuFd uintptr, event *VCPUEvents) error {
	_, err := Ioctl(vcpuFd, IIOW(kvmSetVCPUEvents, unsafe.Sizeof(VCPUEvents{})), uintptr(unsafe.Pointer(event)))
